...
def setup_application():
            ...
            if not SettingsService:
                class DummySettingsService:
                    ...
                    def save_settings(self):
                        pass
                settings_service = DummySettingsService(config_store)
            ...
            if not ProfileService:
                class DummyProfileService:
                    ...
                    def get_default_profile(self):
                        return None
                profile_service = DummyProfileService(profile_manager, config_store)
            ...
            if not ImportService:
                class DummyImportService:
                    ...
                    def get_all_imports(self):
                        return []
                import_service = DummyImportService()
    ...
    # Log service status
    logger.info(f"Services initialized - settings: {'OK' if settings_service else 'FAILED'}, " +
                f"profile: {'OK' if profile_service else 'FAILED'}, " +
                f"import: {'OK' if import_service else 'FAILED'}, " +
                f"update: {'OK' if update_service else 'FAILED'}")
